...
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐            │  │
│  │   Auth   │ │  Notes   │ │ Folders  │ │  Subscription│            │  │
│  └──────────┘ └──────────┘ └──────────┘ └──────────────┘            │  │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐            │  │
...
> **When building any UI, use `Ui_sample.html` (in the project) as the visual source of truth.** It is the canonical reference for the look and feel of the user-facing app; developers should open it and match its layout, components, spacing, and styling. It is an exploration and not yet complete — where it doesn't cover a screen, follow its established design language rather than inventing a new one.
...
| Database | Liara PostgreSQL (managed) | Auto-backup enabled; pgvector extension enabled |
...
Admin ──< AuditLog        (admin actions over Users / Plans / Subscriptions)
 └──< AdminOtp            (two-step login codes)
```

> **Note:** `AuditLog`, `Subscription.grantedByAdminId`, and the manual `ADMIN_ADJUSTMENT` credit transaction type are the links between an `Admin` and the user-side records they act on. `PaymentRecord` was previously referenced only implicitly in the payment flow (Section 12) and is now a first-class model, required by the backoffice payments view.
...
model Subscription {
  ...
  startedAt       DateTime
  expiresAt       DateTime
  grantedByAdminId String?           // set when an admin manually granted this subscription (comp); null = paid via gateway
  cancelReason    String?            // set when an admin cancels the subscription
  createdAt       DateTime           @default(now())
...
model Admin {
  ...
  email         String      @unique
  passwordHash  String
  mobile        String                  // OTP (step 2) is sent here
  displayName   String
...
enum AdminRole {
  ...
  SUPPORT       // view users, adjust credits, grant/cancel subscriptions, impersonate; no plan/payment edits, no admin management
}
...
model AuditLog {
  ...
  adminId     String
  action      AuditAction
  targetType  String                  // "USER" | "PLAN" | "SUBSCRIPTION" | "ADMIN"
  targetId    String?
...
1. Generate a vector embedding for the user's query via the Metis embeddings endpoint (Section 6.0)
2. Perform a cosine similarity search in pgvector on the `NoteChunk` table, filtered by the notes belonging to the session's `selectedNoteIds` (which are within the selected `folderId`)
3. Retrieve the top 5 most relevant chunks
4. Send the RAG chatbot system prompt + retrieved chunks + user query to Gemini via Metis (`generateContent`)
5. Return the AI response and save both messages (user + assistant) to `ChatMessage`
...
### 5.7 Subscription & Payment Operations
...
type Mutation {
  initiatePayment(planId: ID!): PaymentLink!
}
...
```graphql
type Mutation {
  adminLoginStep1(email: String!, password: String!): AdminLoginChallenge!   # [Public]
...
type AdminLoginChallenge {
  ...
  maskedMobile: String! # e.g. "0912****567" — where the OTP was sent
}
...
> Step 1 always returns success-shaped responses for valid input format and **does not reveal whether the email exists**; on bad credentials it returns the standard `ADMIN_CREDENTIALS_INVALID` error without distinguishing "no such email" from "wrong password". The OTP is only sent when credentials are valid.
...
#### 5.9.3 Per-User Subscription Management

```graphql
type Mutation {
  adminGrantSubscription(input: GrantSubscriptionInput!): UserSubscription!  # [SUPPORT]
  adminCancelSubscription(input: CancelSubscriptionInput!): Boolean!         # [SUPPORT]
}

input GrantSubscriptionInput {
  userId: ID!
  planId: ID!
  expiresAt: DateTime    # optional override; default computed from plan.durationDays
}
...
`adminGrantSubscription` creates a `Subscription` (status ACTIVE, `grantedByAdminId` set), applies the plan's credit allowance via a `CreditTransaction` (type `ADMIN_ADJUSTMENT`), and publishes `creditUpdated` to the user. `adminCancelSubscription` sets status `CANCELLED` and `cancelReason`; it does **not** reverse already-granted credits.

#### 5.9.4 Plan Management
...
input AdminPlanInput {
  name: String!
  type: PlanType!        # SUBSCRIPTION | CREDIT_PACK
  priceIRT: Int!
  credits: Int!
  durationDays: Int      # required for SUBSCRIPTION, null for CREDIT_PACK
}
```
...
#### 5.9.8 Admin Management (SUPER_ADMIN only)
...
#### 5.9.9 Platform Settings — OTP Channels
...
|---|---|
| From inside Iran (Liara production) | `https://api.metisai.ir` |
| From sanctioned / blocked networks (e.g. CI on Colab/Kaggle) | `https://api.tapsage.com` |

Configured via `METIS_BASE_URL` (default `https://api.metisai.ir`). A single Metis API key (prefix `tpsg-`) is used for every call, stored as `METIS_API_KEY`.
...
2. API Server:
   ...
   c. Generate S3 presigned PUT URLs (10-min expiry) with Content-Type and Content-Length conditions
...
| New user free grant | 20 credits (configurable via env) |
...
- **Both plan types add to the same balance.** A `CREDIT_PACK` is a one-time top-up. A `SUBSCRIPTION` is simply a larger, discounted credit bundle (ideally aligned to an academic term); on purchase it grants its `credits` to the same balance and creates a `Subscription` record used only to show "active plan until {expiresAt}" and to send a renewal reminder.
- **No automatic recurring charge.** Renewal is a **manual repurchase** by the user (prompted by a reminder near term end), not an auto-debit. This matches the default Zarinpal one-off redirect flow and avoids charging seasonal users in months they don't use the app. (Zarinpal Direct Debit auto-renew can be added later as an opt-in if predictable recurring revenue becomes a priority.)
- **Subscription expiry is cosmetic, not punitive.** When a `Subscription` reaches `expiresAt`, its status becomes `EXPIRED` (the "active plan" badge drops and a renewal reminder is sent), but **the user keeps any unused credits** — expiry never removes balance.
...
- The backoffice analytics `processingSuccessRate` (Section 5.9.6) is the product-level view of the same signal
...
OTP can be delivered over two channels, and the **admin chooses the mode** in the backoffice (Section 5.9.9):
...
- **Phone format:** `09xxxxxxxxx` / `989xxxxxxxxx` accepted.
...
- **Phone format:** must be `98XXXXXXXXXX` — international form, **no leading zero**. Neviso normalizes the stored mobile (`0912…` → `98912…`) before calling Bale.
...
```
1. mutation adminLoginStep1(email, password)
   → Look up Admin by email
   → Verify bcrypt(password); verify isActive
   → On any failure: return ADMIN_CREDENTIALS_INVALID (no email-existence leak)
...
|---|---|---|
| Audio | 200 MB | mp3, m4a, ogg, wav, webm |
| Image | 10 MB each | jpg, jpeg, png, webp, heic |
| Max images per upload | 10 files | — |
...
1. mutation initiatePayment(planId)
   ...
   → Call Zarinpal: POST /pg/v4/payment/request.json
   → Store authority token
   → Return { paymentUrl }

2. User completes payment on Zarinpal

3. GET /api/payments/verify?Authority=...&Status=OK  [REST — external redirect]
   → Call Zarinpal: POST /pg/v4/payment/verify.json
   → On success:
      a. PaymentRecord → PAID
      b. Add credits (CreditTransaction: PURCHASE)
      c. If SUBSCRIPTION plan → create Subscription record
      d. Publish to creditUpdated subscription
   → Redirect to /dashboard?payment=success

4. On failure → redirect to /dashboard?payment=failed
```

> **Note:** Step 3 is the only REST endpoint in the system. It exists because Zarinpal redirects the user's browser to this URL after payment — it cannot be a GraphQL operation.
...
```json
{
  ...
  "amount": 150000,
  "currency": "IRT",
  "description": "خرید اشتراک نویسو - پلن ماهانه",
...
| Presigned URL Validation | Presigned PUT URLs enforce `Content-Type` condition (allowlist: audio/mpeg, audio/mp4, audio/ogg, audio/wav, audio/webm, image/jpeg, image/png, image/webp, image/heic) and `Content-Length` condition (max per file type limits); 10-minute expiry; unique key per file prevents reuse |
...
| Admin 2FA | Mandatory two-step login (password → SMS OTP); step 1 does not leak email existence; OTP single-use, 5-min TTL |
| Admin session lifetime | Short access (15m) and refresh (8h) TTLs; refresh rotation; idle re-login |
| Admin login rate limiting | `adminLoginStep1`: max 5 attempts / 15 min per email + per IP; repeated bad OTP invalidates the challenge |
...
METIS_BASE_URL=https://api.metisai.ir   # use https://api.tapsage.com from sanctioned/blocked networks
...
| `ADMIN_CREDENTIALS_INVALID` | ایمیل یا رمز عبور نادرست است. | inline (admin) |
...
| `PLAN_IN_USE` | این پلن قابل حذف نیست؛ به‌جای حذف، آن را غیرفعال کنید. | toast (admin) |
...
- **Gemini audio limits:** Audio length per request is bounded by the chosen Gemini model (e.g. Gemini 1.5 Pro ~8.5 hours). The model is configurable via `METIS_GEMINI_MODEL`; a chunking strategy must be defined for recordings longer than the active model's limit.
...
- **Subscription expiry:** ~~Resolved~~ — a nightly BullMQ cron marks subscriptions `EXPIRED` past `expiresAt` and triggers a renewal reminder. Expiry is cosmetic: it only drops the "active plan" status and never removes credits (credits never expire; see Section 6.3). Renewal is a manual repurchase, not an auto-charge.
